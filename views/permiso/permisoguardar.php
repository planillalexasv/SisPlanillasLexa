...
session_start();
    ...
    $anio = date("Y");
		...
		$insertplanilla = "INSERT INTO planilla(IdEmpleado, FechaTransaccion, Permisos, DiasPermiso,MesPlanilla, AnioPlanilla)"
		 									 . "VALUES ('$Empleado','$Fecha', '$SalarioDescuento','$Dias', '$mes', '$anio')";
		$resultadoinsertplanilla = $mysqli->query($insertplanilla);

    $insert = "INSERT INTO permiso(IdEmpleado,DiasPermiso,SalarioDescuento,FechaPermiso,
                                         PeriodoPermiso,MesPermiso,DescripcionPermiso)"
                            . "VALUES ('$Empleado','$Dias', '$SalarioDescuento' , '$Fecha',
                                    '$mes','$anio','$Motivo')";
    $resultadoinsert = $mysqli->query($insert);
    $last_id = $mysqli->insert_id;

    header('Location: ../../web/permiso/view?id='.$last_id);
		  // echo $insertplanilla;
?>
